...
export default function PredictionsPage() {
  ...
  useEffect(() => {
    const compute = async () => {
      ...
      try {
        const res = await predictionService.computeInteractive(selectedCity, features);
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex items-center gap-4 px-6 py-3 bg-white/5 border border-white/10 rounded-2xl">
           <div className="text-right">
              <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Confiance Globale</div>
              <div className="text-lg font-black text-white">96.8%</div>
           </div>
           <div className="w-px h-8 bg-white/10" />
           <div className="flex gap-1">
              {[1,2,3,4,5].map(i => <div key={i} className={`w-1 h-4 rounded-full ${i <= 4 ? 'bg-[#00d4b1]' : 'bg-white/10'}`} />)}
           </div>
        </div>
      </header>
