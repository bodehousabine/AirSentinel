...
from utils import load_data, get_img_as_base64, render_transition_loader
...
# ── 0. GESTION DE LA TRANSITION VERS LE DASHBOARD ──────────────────────────
if st.session_state.get("loading_dashboard"):
    is_first = st.session_state.get("first_dashboard_load", True)
    dur = 6.0 if is_first else 2.5
    
    t0 = time.time()
    render_transition_loader(st.session_state.get("lang", "fr"), duration=dur)
    
    # 🔮 PRÉCHAUFFAGE DU CACHE (Pre-warming) 
    try:
        _df = load_data()
        if _df is not None:
            _date_max_str = str(_df["date"].max().date())
            from blocs.bloc1_carte import _predire_toutes_villes
            _predire_toutes_villes(_date_max_str, _df)
    except:
        pass
    
    # Calculer le temps restant pour que l'animation finisse PILE au moment du switch
    elapsed = time.time() - t0
    remaining = max(0.1, dur - elapsed)
    time.sleep(remaining)
    
    st.session_state["first_dashboard_load"] = False
...
if st.session_state["page"] == "landing":
    ...
    if entered:
        st.session_state["loading_dashboard"] = True
...
.stTabs .stTabs [aria-selected="true"] {{
    background: {th['border_soft']} !important;
    color: {th['teal'] if th['name']=='dark' else '#006b58'} !important;
    border-bottom: 3.5px solid {th['teal']} !important;
}}
